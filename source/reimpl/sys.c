...
// Il gioco pilota le animazioni (fade di alpha della UI) con gettimeofday.
// Il boilerplate lo mappava alla gettimeofday di newlib, che su Vita non e'
// agganciata all'RTC: qui si usa la stessa sorgente di clock_gettime REALTIME.
int gettimeofday_soloader(struct timeval * tv, void * tz) {
    ...
    tv->tv_sec = (long)(tick.tick / 1000000);
    tv->tv_usec = (long)(tick.tick - ((uint64_t)tv->tv_sec * 1000000));

    // Sonda: primi campioni e delta, per verificare che il tempo scorra.
    static int budget = 12;
    static uint64_t prev = 0;
    if (budget > 0) {
        budget--;
        uint64_t now = tick.tick;
        l_debug("gettimeofday -> sec=%li usec=%li (delta %llu us)",
                tv->tv_sec, tv->tv_usec, prev ? (now - prev) : 0);
        prev = now;
    }
    return 0;
}
